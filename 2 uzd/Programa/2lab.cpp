...
//            return(value);
    ...
    void outputQueueToScreen ()
    {
        int i;
        if(isEmpty()) {
            cout << endl << "Queue is Empty, nothing to output!" << endl;
        }
        else {
//            cout << endl << "start = " << start;
            cout << endl << "Elements in the Queue: "<<endl;
            for(i=start; i<=end; i++)
...
//            cout << endl << "end = " << end << endl;
    ...
    void outputQueueToFile ()
    {
        ...
        ofstream out(fln);
        if(isEmpty()) {
            out<<endl<<"Queue is Empty, nothing to output!" << endl;
        }
        else {
//            cout << endl << "start = " << start;
            out << endl << "Elements in the Queue: "<<endl;
            for(i=start; i<=end; i++)
...
double n;
    ...
    while (1) {
        cout << endl << endl;
        cout <<"Possible choices: " << endl << endl << "1. Enter a decimle number and convert it to a binary representation (result will be shown in the screen console); " << endl << "2. Print all results into a textfile; " << endl << "3. Remove an existing element from the queue; " << endl << "Any other key to exit application."<<endl;
        cin>>choice;
        if(choice==1) {
            cout << "Enter a decimal fraction:     ";
            cin>>n;
            cout <<endl<< "How many numbers would you like to see after the decimal separator?     ";
            cin>>pl;
            q.enQueue_num(n); q.enQueue_bin(decToBin(n, pl));
